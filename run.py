...
parser.add_argument('--low_k', type=int, default=10)
parser.add_argument('--mid_k', type=int, default=10)
...
if __name__ == '__main__':
    ...
    #load dataset
    # feature: 类别嵌入，拼接了cid2_emb和cid3_emb
    # price_bin: 整数的价格(原本是连续的，分箱后变为离散的)
    # com_edge_index: 共边的训练下标
    # train_set: 共边训练下 标 + 2个负样本下标
    # val_set: 共边验证下标 + 100个负样本下标
    # test_set: 共边测试下标 + 100个负样本下标
    features, price_bin, com_edge_index, train_set, val_set, test_set = load_dataset(args.dataset)

    num_items = features.shape[0]  # 商品数量
    adj, adj_self, adj_dele = generate_adj(com_edge_index, num_items)   # 卷积核
    ...
    for run in range(args.runs):
        ...
        model = SComGNN(args.embedding_dim, 20, args.mode, args.low_k, args.mid_k, args.alpha, args.beta, args.dataset)
