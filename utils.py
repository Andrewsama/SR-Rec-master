...
def generate_adj(edge_index, num_items):
    ...
    adj_norm_self = adj_norm + sp.eye(adj_norm.shape[0])
    adj_norm_dele = adj_norm - sp.eye(adj_norm.shape[0])
    adj = sparse_mx_to_torch_sparse_tensor(adj)
    adj_norm = sparse_mx_to_torch_sparse_tensor(adj_norm)
    adj_norm_self = sparse_mx_to_torch_sparse_tensor(adj_norm_self)
    adj_norm_dele = sparse_mx_to_torch_sparse_tensor(adj_norm_dele)
    print("Finish generating adjacent matrix")

    return adj, adj_norm_self, adj_norm_dele
